...
Page(Object.assign({}, Zan.Toast, {
  ...
  fetchNewData: function () {  //获取最新列表
      ...
      wx.request({
        url: 'https://shop.playonwechat.com/api/get-carts?sign=' + app.data.sign + '&operator_id=' + app.data.kid,
        method: "GET",
    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          ...
          //移除列表中下标为index的项  
           wx.request({
             url: "https://shop.playonwechat.com/api/remove-cart-by-key?sign=" + app.data.sign + '&operator_id=' + app.data.kid,
             header: {
  ...
  seting: function (e) {
    ...
    if (ids == 1) {
      ...
      if (change_carts.length != 0){
        wx.request({
          url: "https://shop.playonwechat.com/api/carts-manage?sign=" + app.data.sign + '&operator_id=' + app.data.kid,
          method: "POST",
          data: {
